...
module.exports = function createReader(deps) {
    ...
    return function startReader(sessionKey, opts) {
        ...
        if (sessionKey.endsWith(':claude')) {
            ...
            // Attach to a JSONL file — sets path, resets position, notifies clients
            function attachToJsonl(filePath) {
                ...
                // Notify all connected clients which JSONL is active
            ...
            entry._clearSession = () => {
                ...
                // Notify clients the slate is clean
                for (const ws of entry.subscribers) {
                    try {
                        if (ws.readyState === WebSocket.OPEN) {
                            ws.send(JSON.stringify({ type: 'jsonl-cleared' }));
                        }
                    } catch (e) { /* ignore */ }
                }
            ...
            dirWatcher = fs.watch(convDir, (eventType, filename) => {
                ...
                if (!existingFiles.has(filename)) {
                    ...
                    // Current file modified — debounce reads (Claude writes many lines rapidly)
                    if (!_readTimer) {
                        _readTimer = setTimeout(() => { _readTimer = null; readNewLines(); }, 30);
                    }
                }
            });
